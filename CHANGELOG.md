...
### Changed
- デフォルトのルーティングファイル名をsite/Route.tsに変更

## [0.1.0-alpha.45] - 2026-06-01

### Fixed
- Fixed an issue where CSS for client scripts was not generated in build mode.
...
### Added
- Implemented Fragment syntax with `<></>` shorthand.
- Added missing `menu` and `param` HTML elements.

### Changed
- Renamed `<unwrap>` element to `<fragment>`.
